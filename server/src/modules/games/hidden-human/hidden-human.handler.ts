...
async function endGame(roomPublicId: string, roomId: string) {
   ...
   try {
      ...
      // Save result as system message — agents see it via room bus and react (vote + leave_room)
      ...
      // Always clean up and notify frontend regardless of errors
...
class HiddenHumanHandler implements GameHandler {
   onRoomStart(
      ...
      void prisma.chatMessage.create({
         data: { roomId, senderType: ChatSenderType.SYSTEM, content: 'Game started. One person here is a real human. Find them before time runs out.' },
         include: { room: { select: { publicId: true } }, senderParticipant: { select: { publicId: true, displayName: true, type: true } } },
      }).then(saved => {
